...
class StaticPlot(QtGui.QMainWindow):
    def __init__(self, parent):
        ...
        self.t2mCheck        = QtGui.QCheckBox('t2m')
        ...
        self.checkBLayout.addWidget(self.t2mCheck)
    ...
    def run(self):
        ...
        if len(markers) > 0:
            ...
            if self.t2mCheck.checkState() == 2:
                dataSetsToPopulate.append('t2m')
            ...
            if self.t2mCheck.checkState() == 2:
                self.t2mPlt = self.plt3.getPlotItem().plot(t2m.distanceData, t2m.pathData, pen=t2m.pen)
                self.legend3.addItem(self.t2mPlt, 't2m')
                t2m.pathPlotItem.setData(t2m.distanceData, t2m.pathData)
