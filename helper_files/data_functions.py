...
def interpolateData(runModel, dr, dataSetsToPopulate):
    ...
    for i in range(1, len(markers)):
        ...
        if runModel or 't2m' in dataSetsToPopulate:
            localT2m = t2m.interp(px,py, grid=False)
            t2mValues.append(localT2m)
        ...
        if runModel or 't2m' in dataSetsToPopulate:
            t2m.pathData = np.array(t2mValues[0])
        ...
        for i in range(1, len(velValues)):
            ...
            if runModel or 't2m' in dataSetsToPopulate:
                t2m.pathData = np.append(t2m.pathData, t2mValues[i])
