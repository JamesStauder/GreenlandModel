...
def createt2m():
    tData = h5py.File('../data/dataCMValues.h5', 'r+')
    t2m = tData['t2m'][:]

    t2mVals = getCM('t2m').mapToByte(t2m)
    t2mVals = t2mVals.astype(np.uint8)
    tData.__delitem__('t2m')
    tData.create_dataset('t2m', data = t2mVals)
    tData.close()
    print('done')
def createCMData():
    subSample = 1
    ...
    t2m = allData['t2m'][:][::subSample, ::subSample]
    allData.close()
    ...
    outF.create_dataset('t2m', data = t2mVals)
    outF.close()
